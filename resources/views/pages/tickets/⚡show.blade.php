...
<section class="space-y-6">
    ...
    <div class="space-y-5">
        <form action="{{route('tickets.ai.triager', $ticket)}}" method="post">
            @csrf
    ...
    <div x-data="ticketDraftDemo({{ $ticket->id }})"
         class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex justify-between">
            <flux:heading size="sm"> Draft Reply</flux:heading>
            <flux:text class="text-xs text-zinc-500 dark:text-zinc-400">
                {{ __('Stream a reply before sending.') }}
            </flux:text>
        </div>
        <div class="mt-4 rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-800">
            <div class="whitespace-pre-line" x-text="draft || '{{ __('Click draft reply') }}'"></div>
        </div>
       ...
       <div class="mt-4 flex items-center gap-3">
           ...
           <flux:button variant="filled" type="button" @click="insertIntoReply">
